...
function ToDoAddForm({ addToDoHandle, closeModal }: ToDoAddFormProps) {
    return (
        <form
            ...
            action=""
            className="shadow-md absolute top-10 left-1/2 -translate-x-1/2 rounded-xl px-8 py-20 bg-dblue bg-opacity-[0.75] backdrop-blur-sm flex flex-col gap-12 z-[9000]"
        >
            <button
                type="button"
                onClick={closeModal}
            ...
            <select
                name="status"
                className="appearance-none border border-mblue rounded-lg text-center font-bold text-mblue  bg-dblue outline-none shadow-md py-4 focus-within:shadow-mblue transition-shadow duration-75"
                defaultValue="to-do"
            >
